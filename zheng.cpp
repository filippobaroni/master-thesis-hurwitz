...
auto compute_r_and_s(T d) {
    ...
    timer::start("r and s");
    std::vector<std::tuple<I, std::vector<std::tuple<I, uint32_t>>>> ans;
    iterate_on_secondary_partitions(p_table, d, [&](auto omega) {
        uint32_t k = omega.size();
        // Compute r(omega)
        I r = (k % 2) ? (1) : (-1);
        ...
        for(const auto& [i, j] : omega) {
            ...
            s = mult_two_polynomials(mult, s, p); // FASTER
        }
        /*std::vector<multivariate_polynomial<I, T>> polys;
        for(const auto& [i, j] : omega) {
            multivariate_polynomial<I, T> p = { i, {} };
            std::get<1>(p).reserve(p_table[i].size());
            for(uint32_t nu = 0; nu < p_table[i].size(); ++nu) {
                std::get<1>(p).emplace_back(scoeff[i][j][nu], nu);
            }
        ...
        auto s = mult_many_polynomials(mult, std::move(polys));*/
        std::get<1>(s).erase(std::get<1>(s).begin());
        std::get<1>(s).shrink_to_fit();
        // Return
        ans.emplace_back(r, std::get<1>(s));
    });
    ans.shrink_to_fit();
    timer::end("r and s");
    return ans;
        ...
        threads.emplace_back([&, t]() {
            while(true) {
                uint32_t myp;
                {
                    ...
                    std::cerr << myp << " / " << P.size() << "    in " << timer::elapsed("compute") << "s" << std::endl;
                }
                ...
                RS.iterate([&](I r, const auto& s) {
                    ...
                    for(uint32_t i0 = 0; i0 < s.size(); ++i0) {
                        if(std::get<1>(s[i0]) < myp or std::get<1>(s[i0]) >= myp + CHUNK_SIZE) {
                            continue;
                        }
                        p[0] = std::get<1>(s[i0]) - myp;
                        sum_len = PS[p[0] + myp];
