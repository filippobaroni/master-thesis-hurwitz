...
\item A graph embedded in $\sphere{}$ whose vertices have degrees $[1,2,\ldots,2,3]$ can be represented by a diagram like the following, where $a$ denotes the number of edges on the path connecting the vertex of degree $3$ to the vertex of degree $1$.
...
\item There are only three kinds of graphs embedded in $\sphere$ whose vertices have degrees $[1,\ldots,1,k+1,k+2]$. We label the vertices of degrees $k+1$, $k+2$ with the letters $u$, $v$ respectively.
\begin{dessintext}
\makecell[c]{
