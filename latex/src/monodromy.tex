...
\begin{remark}
We can extract some additional information from the statement [and the proof] of \cref{combinatorial-move:a:large-v}. First of all, since both $\DD$ and $\DD'$ are candidate data, their total branching number must be even, so $v(\pi_1')\equiv v(\pi_1)+v(\pi_2)\pmod{2}$. Moreover, $g'$ can be explicitly computed by means of the \RH{} formula:
\[
...
\begin{itemize}
\item Assume that $v(\pi_1)+v(\pi_2)\le d-1$. \Cref{combinatorial-move:a:small-v} gives
\[
...
There are two cases.
\begin{itemize}
\item Assume that the inequality $v(\pi_i)+v(\pi_j)\le d-1$ holds for a pair of indices $1\le i<j\le n-1$; up to reindexing, we can assume that $i=1$ and $j=2$. \Cref{combinatorial-move:a:small-v} gives
\[
...
\item Otherwise, the inequality $v(\pi_1)+v(\pi_j)\ge d$ holds for every $1\le i<j\le n-1$. In particular, $v(\pi_1)+v(\pi_2)\ge d$ and $v(\pi_3)+v([1,d-1])\ge d-1$. \Cref{combinatorial-move:a:large-v} gives
\[
\DD\cmove\DD'=\datum{\surf{g'}}{d}{\pi_1',\pi_3,\ldots,\pi_{n-1},[1,d-1]}
\]
