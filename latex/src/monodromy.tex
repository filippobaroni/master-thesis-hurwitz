...
\section{Projective plane and sphere}
...
\section{First results on the sphere}
...
Assume now that the statement holds for candidate data with at most $n-1$ partitions, and fix $\DD=\datum{\surf{g},\sphere{}}{d}{\pi_1,\ldots,\pi_{n-1},[d]}$. There are two cases.
\begin{itemize}
...
\begin{proposition}
Let $\DD=\datum{\surf{g},\sphere{}}{d}{\pi_1,\ldots,\pi_{n-1},[1,d-1]}$ be a proper candidate datum. Then $\DD$ is not realizable if and only if it satisfies one of the following.
\begin{enumerate}[(1)]
...
\[
g'=\frac{1}{2}(v(\alpha_1\alpha_2)+v(\pi_3)+\ldots+v(\pi_n))-d+1\ge\frac{1}{2}(d-2+d)-d+1\ge 0.
\]
It is easy to see that $\DD'=\datum{\surf{g'},\sphere{}}{d}{[\alpha_1\alpha_2],\pi_3,\ldots,\pi_n}$ is a candidate datum, so it is realizable by induction. Similarly to the previous case, this implies that $\DD$ is realizable.\qedhere
\end{itemize}
\end{proof}