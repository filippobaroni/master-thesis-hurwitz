...
Let $\alpha\in\symgroup[d]$ be a permutation. Set $r=d-v(\alpha)$, and let $t\ge 0$ be an integer such that $2t\le v(\alpha)$. Then $\alpha$ can be written as the product of an $(r+2t)$\=/cycle and a $d$\=/cycle.
\end{lemma}
\begin{proof}
...
\end{align*}
where $b_1<b_2<\ldots<b_{2t}$ are elements of $\{1,\ldots,d\}\setminus\{1,d_1+1,\ldots,d_{r-1}+1\}$; note that there are at least $2t$ elements in this set, since $d-r=v(\alpha)\ge 2t$. Set
\[
...
Let $X,Y$ be finite sets; denote by $h$ the cardinality of $Y$, and by $k$ the cardinality of $X\cap Y$. Let $\beta=\cycle{b_1,\ldots,b_h}\in\symgroup(Y)$ be an $h$\=/cycle, and let $\gamma\in\symgroup(X\cap Y)$ be a $k$\=/cycle of the form $\gamma=\cycle{b_{i_1},\ldots,b_{i_k}}$ with $1\le i_1<\ldots<i_k\le h$. Then, for every $\alpha\in\symgroup(X)$, we have that $\alpha$ and $\alpha\gamma^{-1}\beta\in\symgroup(X\cup Y)$ have the same number of cycles.
\end{lemma}
\begin{proof}
...
\]
hence $a_1\le s$. Fix $\alpha_1=\cycle{1,d_1+1,\ldots,d_{a_1-1}+1}$, and let $A=\{1,d_1+1,\ldots,d_{a_1-1}+1\}$ be the support of $\alpha_1$. Define $Q=\{1,\ldots,d_{a_1}\}\setminus A$; note that $Q$ is non-empty, since $d_1+1\ge 3$ implies that $2\in Q$. Consider the partitions of $d-a_1$
\begin{align*}
\pi'=[a_2,a_3,\ldots,a_r],&&\rho'=[\card{Q},b_{a_1+1},\ldots,b_s].
\end{align*}
We have that
\begin{align*}
v(\pi')=v(\pi)-a_1+1,&&v(\rho')=v(\rho)-1.
\end{align*}
...
\[
\alpha\beta=\cycle{1,3,\ldots,d-1}\cycle{d,d-2,\ldots,2}.
\]
...
\begin{itemize}
\item If $v(\alpha)+v(\pi_i)<d$, by \cref{monodromy:th:product-reduction-small-v} we can find $\alpha_i\in\symgroup[d]$ with $[\alpha_i]=\pi_i$ such that $v(\alpha\alpha_i)=v(\alpha)+v(\alpha_i)$. The property is still satisfied: if $v(\alpha)=v(\pi_1)+\ldots+v(\pi_{i-1})$ then obviously $v(\alpha\alpha_i)=v(\pi_1)+\ldots+v(\pi_i)$. If instead $[\alpha]\in\{[d],[1,d-1],[d/2,d/2]\}$, then either $\alpha_i$ is the identity, or $\alpha\alpha_i$ is a $d$\=/cycle; either way, $[\alpha\alpha_i]\in\{[d],[1,d-1],[d/2,d/2]\}$. Note that if the action of $\angled{\alpha_1,\ldots,\alpha_{i-1}}$ is transitive, then the action of $\angled{\alpha_1,\ldots,\alpha_i}$ is transitive as well.
\item If $v(\alpha)+v(\alpha_i)\ge d$, \cref{monodromy:th:product-reduction-large-v} gives a permutation $\alpha_i\in\symgroup[d]$ with $[\alpha_i]=\pi_i$ such that $[\alpha\alpha_i]\in\{[d],[1,d-1],[d/2,d/2]\}$ and the action of $\angled{\alpha,\alpha_i}$ is transitive. The property is obviously satisfied.
\end{itemize}
The last step of the induction yields permutations $\alpha_1,\ldots,\alpha_n\in\symgroup[d]$ with $[\alpha_i]=\pi_i$ such that
\[
[\alpha_1\cdots\alpha_n]\in\{[d],[1,d-1],[d/2,d/2]\}
\]
and $\angled{\alpha_1,\ldots,\alpha_n}$ acts transitively on $\{1,\ldots,d\}$ (note that $v(\alpha_1\cdots\alpha_n)=v(\alpha_1)+\ldots+v(\alpha_n)$ implies that $[\alpha_1\cdots\alpha_n]=[d]$). By \cref{hurwitz:th:branching-number-permutations-product} we have that
\[
v(\alpha)\equiv v(\alpha_1\cdots\alpha_n)\equiv v(\alpha_1)+\ldots+v(\alpha_n)\pmod{2},
\]
which is even since $\DD$ is a candidate datum. We now prove that $\alpha=\alpha_1\cdots\alpha_n$ is a square.
\begin{itemize}
\item If $[\alpha]=[d]$, then $v(\alpha)\equiv d-1\equiv 0\pmod{2}$. This implies that $d$ is odd, so $\alpha$ is the square of $\alpha^{(d+1)/2}$.
\item If $[\alpha]=[1,d-1]$, then $v(\alpha)\equiv d-2\equiv 0\pmod{2}$. This implies that $d$ is even, so $\alpha$ is the square of $\alpha^{d/2}$.
\item If $[\alpha]=[d/2,d/2]$, then $d$ is even, and it is easy to see that $\alpha$ is the square of a $d$-cycle.
\end{itemize}
In any case, we obtain a permutation $\beta_1\in\symgroup[d]$ such that $\alpha=\beta_1^{-2}$. By \cref{hurwitz:th:monodromy-realizability-non-orientable}, this implies that $\DD$ is realizable; to verify condition \ref{hurwitz:it:monodomy-realizability-non-orientable-fourth}, simply note that there exists a permutation $\gamma\in\angled{\alpha_1,\ldots,\alpha_n}$ such that $\gamma\beta_1$ has a fixed point, since $\angled{\alpha_1,\ldots,\alpha_n}$ acts transitively on $\{1,\ldots,d\}$.
\end{proof}
...
\end{itemize}
Moreover, \cref{hurwitz:th:monodromy-realizability-double-covering} shows that the cases where $\Sigma=\RP[2]$ and $\tSigma$ is orientable can be reduced to the analysis of realizable candidate data with $\Sigma=\sphere$. As a consequence, it stands to reason that, from now on, we will only be concerned with candidate data having the sphere as the base surface; in fact, a classification of the realizable data of this kind would lead to a full solution of the Hurwitz existence problem. However, we will soon find out that realizability in the case $\Sigma=\sphere{}$ can be a very delicate matter: there are several seemingly unrelated families of exceptional data, and even to this day we are still far from a complete understanding of the problem.
%To be fair, even after solving the existence problem on the sphere, compiling the full list of exceptional data with $\Sigma=\RP[2]$ and $\tSigma$ orientable could still be non-trivial.

Since, from now on, we will only deal with branched coverings with the sphere as the base surface, we adopt the following convention: whenever we have a combinatorial datum $\DD=\datum{\surf{g},\sphere{}}{d}{\pi_1,\ldots\pi_n}$, we can omit the $\sphere{}$ and simply write $\DD=\datum{\surf{g}}{d}{\pi_1,\ldots\pi_n}$.
...
\begin{definition}
Let $\DD,\DD'$ be combinatorial data. We say that there is a \emph{combinatorial move} between them, and we write $\DD\cmove\DD'$, if the realizability of $\DD'$ implies that of $\DD$.
\end{definition}
...
\end{assumptions}
Then there exist an orientable surface $\surf{g'}$ and a partition $\pi_1'\in\Partitions{d}$ such that $\DD'=\datum{\surf{g'}}{d}{\pi_1',\pi_3,\ldots,\pi_n}$ is a candidate datum and $\DD\cmove\DD'$. Moreover, if $\pi_1=\pi_2=[2,\ldots,2]$ then $\pi_1'$ can be chosen equal to $[d/2,d/2]$, otherwise it can be chosen in $\{[d],[1,d-1]\}$.
\end{combinatorialmovea}
\begin{proof}
...
\]
Note that $v(\pi_1')\ge d-2$ and $v(\pi_3)+\ldots+v(\pi_n)\ge d-1$, so
\[
...
\]
It follows that $g'$ is a non-negative integer; by definition of $g'$, we see that $\DD'$ is a candidate datum. The very same argument used in the proof of \cref{combinatorial-move:a:small-v} shows that $\DD\cmove\DD'$.
\end{proof}
...
\]
Moreover, the total branching number $v(\pi_1)+v(\pi_2)+d-1$ is even. \Cref{monodromy:th:product-reduction-odd} then gives permutations $\alpha_1,\alpha_2\in\symgroup[d]$ with $[\alpha_1]=\pi_1$ and $[\alpha_2]=\pi_2$ such that $[\alpha_1\alpha_2]=[d]$. By \cref{hurwitz:th:monodromy-realizability-orientable}, $\DD$ is realizable.

We now turn to the case $n\ge 4$. Fix a candidate datum $\DD=\datum{\surf{g},\sphere{}}{d}{\pi_1,\ldots,\pi_{n-1},[d]}$; there are two cases.
\begin{sideline}{Case 1.}
If $v(\pi_1)+v(\pi_2)\le d-1$, \cref{combinatorial-move:a:small-v} gives
\[
...
\begin{sideline}{Case 1.}
If the inequality $v(\pi_i)+v(\pi_j)\le d-1$ holds for a pair of indices $1\le i<j\le n$, it is now routine to employ \cref{combinatorial-move:a:small-v} to show that $\DD$ is realizable by induction.
\end{sideline}
\begin{sideline}{Case 2.}
Otherwise, we have $v(\pi_i)+v(\pi_j)\ge d$ for every $1\le i<j\le n$. We consider two sub-cases.
\begin{itemize}
\item Assume first that there is a partition, say $\pi_1$, which is different from $[2,\ldots,2]$. \Cref{combinatorial-move:a:large-v} gives
\[
...
\section{The prime-degree conjecture}
