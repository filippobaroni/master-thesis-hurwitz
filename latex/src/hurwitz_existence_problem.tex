...
\[
\pi_1(\holed{\surf{g}},x_0)=\langle a_1,\ldots,a_n,b_1,\ldots,b_g,c_1,\ldots,c_g\mid [b_1,c_1]\cdots[b_g,c_g]\cdot a_1\cdots a_n\rangle,
\]
...
\end{enumerate}
In this case, $\tSigma$ is non-orientable if and only if there exists a permutation $\gamma\in\symgroup[d]$ which has a fixed point and can be written as a product of an odd number of $\beta_j$ and any number of $\alpha_i$.
\end{corollary}
\begin{proof}
...
\[
v(\pi_1)+\ldots+v(\pi_n)=v(\alpha_1)+\ldots v(\alpha_n)\equiv v(\alpha_1\cdots\alpha_n)\equiv 0\pmod{2}.
\]
\end{proof}