...
We now introduce the monodromy action in the general setting of topological spaces; we will assume that all the spaces we mention are locally path connected.
...
\Cref{hurwitz:th:monodromy-permutation-matches-partition} gives a group-theoretic criterion for the realizability of a given combinatorial datum.
...
\item There exists a group homomorphism $\map{\psi}{\pi_1(\holed{\Sigma},x_0)}{\op{\symgroup[d]}}$ such that $\im(\psi)\subgroup\op{\symgroup[d]}$ acts transitively on $\{1,\ldots,d\}$ and $[\psi(\gamma_i)]=\pi_i$ for every $1\le i\le n$.
\end{enumerate}
\end{proposition}
\begin{proof}
...
\end{corollary}
\begin{proof}
Let $\holed{\Sigma_g}$ be the non-compact surface obtained by removing $n$ points $x_1,\ldots,x_n$ from $\Sigma_g$. Fix a base-point $x_0\in\holed{\Sigma_g}$. Once we observe that $\pi_1(\holed{\Sigma_g},x_0)$ has a presentation
\[
\pi_1(\holed{\Sigma_g},x_0)=\langle a_1,\ldots,a_n,b_1,\ldots,b_g,c_1,\ldots,c_g|[b_1,c_1]\cdots[b_g,c_g]\cdot a_1\cdots a_n\rangle
\]
where $a_i$ is a loop around $x_i$, the criterion follows immediately from \cref{hurwitz:th:monodromy-realizability-general}. To see that $\tSigma$ is orientable, simply note that a covering space of an orientable surface is orientable, and orientability is not affected by removing a finite number of points.
\end{proof}
...
Given $\nosurf{g}$, $d$ and $\pi_1,\ldots,\pi_n$, there are at most two surfaces $\tSigma$, one orientable and one non-orientable, such that $\DD=\datum{\tSigma,\nosurf{g}}{d}{\pi_1,\ldots,\pi_n}$ is a candidate datum: like in \cref{hurwitz:rm:sigma-tilde-unique-orientable}, the \RH{} formula fixes $\chi(\tSigma)$ which, together with orientability, uniquely determines $\tSigma$.
\end{remark}
