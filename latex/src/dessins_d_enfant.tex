...
\path \surfcirclepoint{d1}{-30} coordinate (1-1) pic{black vertex};
\path \surfcirclepoint{d1}{-150} coordinate (1-3) pic{black vertex};
\path \surfcirclepoint{d1}{-90} coordinate (1-2) pic{black vertex};
\ifnumcomp{#1}{=}{1}{\foreach \i in {1,2,3} {\node[below=1pt] at (1-\i) {$1$};};}{}
}
...
\picturesetupone{#1}
\draw[red] (1-3) to[out=90,in=60,out looseness=3.1,in looseness=2] (1-2);
\path \tuberightpoint{-90} coordinate (tmp);
\draw[red] let \p1=\tuberightpoint{-60},\p2=\tubeleftpoint{-120},\n1={(\x1-\x2)/2} in (1-1) to[bend left] (\p1) arc(0:180:\n1) to[bend right] (1-2);
}
\tabcolsep=0pt
\begin{tabular}{*{3}{|p{.33\linewidth}}|}
\makecell[c]{
