...
\vspace{0.5em}
\begin{itemize}
\item<2-> Se $\map{f}{\tSigma}{\surf{g}}$ è un rivestimento \sidenote<2->{ramificato}{\draw (mymark) to[out=45,in=180] (1,.45) node[right] {in $\{x_1,\ldots,x_n\}$};}, $\pi_1(\holed{\surf{g}},x_0)$ agisce sulla fibra $f^{-1}(x_0)$ \textcolor{black!80}{(monodromia del rivestimento $\map{\holed{f}}{\holed{\tSigma}}{\holed{\surf{g}}}$)}.
\item<3-> Questa azione induce un morfismo di gruppi
\[
\map{\Mon}{\pi_1(\holed{\surf{g}},x_0)}{\op{\symgroup(f^{-1}(x_0))}\iso\symgroup[d]}.
\]
\item<4-> Le lunghezze dei cicli di $\Mon(a_i)$ corrispondono agli elementi di \sidenote<5->{$\pi(x_i)$.}{\draw[shorten <=3pt] (mymark) to[out=30,out looseness=2,in=-30] (.6,1.2) node[anchor=east,align=right] {scriviamo\\$[\Mon(a_i)]=\pi(x_i)$};}
\end{itemize}
\end{frame}
...
\begin{enumroman}
\item<2-> $[\alpha_i]=\pi_i$ per ogni $1\le i\le n$;
\item<3-> $[\beta_1,\gamma_1]\cdots[\beta_g,\gamma_g]\cdot\alpha_1\cdots\alpha_n=1$;
\item<4-> $\angled{\alpha_1,\ldots,\alpha_n,\beta_1,\ldots,\beta_g,\gamma_1,\ldots,\gamma_g}$ agisca \sidenote<5->{transitivamente}{\draw (mymark) to[out=-120,in=30] (-.7,-.5) node[anchor=east] {$\tSigma$ connessa};} su $\{1,\ldots,d\}$.
\end{enumroman}
\end{mybox}
\vspace{.5em}
\visible<6->{
...
\begin{enumroman}
\item<7-> $[\alpha_1]=[\alpha_2]=[2,2]$ e $[\alpha_3]=[1,3]$\sidenote<8->{;}{\draw[shorten <=3pt] (mymark) to[out=0,in=180] (2,.2) node[anchor=west,align=left] {$\alpha_1=\cycle{\bullet,\bullet}\cycle{\bullet,\bullet}$\\$\alpha_2=\cycle{\bullet,\bullet}\cycle{\bullet,\bullet}$\\$\alpha_3=\cycle{\bullet,\bullet,\bullet}$};}
\item<9-> $\alpha_1\alpha_2\alpha_3=1$\sidenote<11->{.}{\draw[shorten <=3pt] (mymark) to (1,0) node[anchor=west] {$\alpha_3=\alpha_2^{-1}\alpha_1^{-1}$};}
\end{enumroman}
\visible<10->{
